# Hello, I’m Mario Tafoya

Aspiring **Software Engineer** focused on building practical tools and solving real-world problems through code.

---

## What I Build
- NLP-driven applications like **ResumeMatcher**
- Interactive desktop applications (GUI)
- Full-stack web applications
- Mobile and cross-platform apps using Flutter
- Machine learning prototypes and data analysis tools
- Embedded systems using microcontrollers

---

## Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C%2FC%2B%2B-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)

### Frameworks & Tools
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Tkinter](https://img.shields.io/badge/Tkinter-FFB000?style=for-the-badge)

### Data & ML
![Scikit--learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)
![NLP](https://img.shields.io/badge/NLP-4B8BBE?style=for-the-badge)

### Other
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

---

## Currently Learning
- Django (backend development)
- AI concepts (Game Theory, Backtracking, Constraint Satisfaction Problems)
- Embedding-based NLP and semantic matching
- Full-stack web and mobile development

---

## Featured Project
- **John Wayne Analytics Data Engineering Pipeline**  
  Designed and built a data pipeline to collect, process, and analyze aviation or operational datasets.  
  - Automated data ingestion and transformation workflows  
  - Cleaned and structured raw data for analysis  
  - Built dashboards and insights using Power BI  
  - Focused on scalability, reliability, and real-world data engineering practices  

---

## GitHub Stats
![Mario's GitHub stats](https://github-readme-stats.vercel.app/api?username=mtafoya-web&show_icons=true&theme=default)

---

## Connect With Me
- GitHub: https://github.com/mtafoya-web
